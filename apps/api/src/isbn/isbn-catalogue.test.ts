...
describe('ISBN catalogue', () => {
  ...
  it('takes the cover from the next catalogue when the first has none', async () => {
    const fetch = fakeFetch({
      [OPEN_LIBRARY_EDITION]: () => Response.json({ title: 'Hobit', publishers: ['Argo'] }),
      [GOOGLE_BOOKS]: googleBooksHit,
    });

    const entry = await createIsbnCatalogue({ fetch }).find(ISBN);

    expect(entry?.details).toMatchObject({ title: 'Hobit', publisher: 'Argo', author: null });
    expect(entry?.coverUrl).toBe('https://books.google.com/books/content?id=x&zoom=1');
  });

  it('asks no further once a catalogue has both the details and a cover', async () => {
    const fetch = fakeFetch({
      [OPEN_LIBRARY_EDITION]: openLibraryEdition,
      [OPEN_LIBRARY_AUTHORS]: openLibraryAuthor,
    });

    await createIsbnCatalogue({ fetch }).find(ISBN);

    expect(fetch.mock.calls.map(([url]) => new URL(String(url)).hostname)).not.toContain(
      'www.googleapis.com'
    );
  });
  ...
  it('answers "unknown" when Open Library lacks the ISBN and Google is out of quota', async () => {
